...
- `pm report` and `pm brief` narrate a period. With no flags they mean "since last time". `--since YYYY-MM-DD`, `--days`, and `--sprint 138` ask for a window. A named window does not move the "last time" memory. `--audience pm` (the default), `leadership`, or `partner` chooses who the file is for. Each audience keeps its own file and its own memory.
